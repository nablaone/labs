...
// {{.Name}} executes query - autogenerated
func (s *Query) {{.Name}}({{range $i, $v := .Ins}}{{if $i}}, {{end}}{{$v.GoName}} {{$v.GoType}}{{end}}) ( error) {
...
// {{.Name}}Row contains single row - autogenerated
...
// {{.Name}} Executes query - autogenerated
func (s *Query) {{.Name}}( {{range $i, $v := .Ins}}{{if $i}}, {{end}}in{{$v.GoName}} {{$v.GoType}}{{end}}) ([]{{.Name}}Row, error) {
...
// _queryQuerier Methods supported by transaction and bare connection
type _queryQuerier interface {
	Exec(query string, args ...interface{}) (sql.Result, error)
	Query(query string, args ...interface{}) (*sql.Rows, error)
}

// SQLTplQ wraps underlying querer
type Query struct {
	q _queryQuerier
}

// WithDB runs queries against database connection
func WithDB(db *sql.DB) *Query {
	return &Query{
		q: db,
	}
}

// WithTX runs queries against database transaction
func WithTX(tx *sql.Tx) *Query {
	return &Query{
		q: tx,
	}
}
